...
const loginAuthor = async function (req, res) {
  try {
    ...
    if (!isValidEmail(Email)) {
      return res
        .status(400)
        .send({ status: false, message: "Email format or pattern is invalid" });
    }
    if (!isValidPassword(Password)) {
      return res
        .status(400)
        .send({
          status: false,
          message:
            "Password should be min 6 and max 20 character.It contains atleast--> 1 Uppercase letter, 1 Lowercase letter, 1 Number, 1 Special character",
        });
    }
